...
Citizen.CreateThread(function()
	local content = LoadResourceFile(GetCurrentResourceName(), "json/bleets.json")
	if not content then
		SaveResourceFile(GetCurrentResourceName(), "json/bleets.json", json.encode({}), -1)
		content = json.encode({})
	end
...
AddEventHandler("bleetSent", function(message, handle, icon, verified, user)
	...
	SaveResourceFile(GetCurrentResourceName(), "json/bleets.json", json.encode(bleets, {indent = true}), -1)
